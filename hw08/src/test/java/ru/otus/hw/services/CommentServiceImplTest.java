...
public class CommentServiceImplTest {
    ...
    void shouldCorrectFindCommentById() {
        Book book =  new Book("1", "title", new Author(), new Genre());
        Comment expectedComment = new Comment("1", "comment_1", book);
        given(commentRepository.findById(eq("1"))).willReturn(Optional.of(expectedComment));
        Optional<Comment> optionalComment = commentService.findById("1");
    ...
    void shouldCorrectSaveComment() {
        ...
        Book book = new Book("1", "title_1", author, genre);
        Comment expectedComment = new Comment(null, "comment_2", book);

        given(bookRepository.findById(eq("1"))).willReturn(Optional.of(book));
